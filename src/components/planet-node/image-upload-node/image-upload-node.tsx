...
import type { NodeViewProps } from "@/planet-core/react"
import { NodeViewWrapper } from "@/planet-core/react"
import { Button } from "@/components/planet-ui-primitive/button"
import { CloseIcon } from "@/components/planet-icons/close-icon"
import "@/components/planet-node/image-upload-node/image-upload-node.scss"
import { focusNextNode, isValidPosition } from "@/lib/planet-utils"
...
const CloudUploadIcon: React.FC = () => (
  <svg
    ...
    height="24"
    viewBox="0 0 24 24"
    className="planet-image-upload-icon"
    fill="currentColor"
...
const FileIcon: React.FC = () => (
  <svg
    ...
    viewBox="0 0 43 57"
    fill="currentColor"
    className="planet-image-upload-dropzone-rect-primary"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
...
const FileCornerIcon: React.FC = () => (
  <svg
    width="10"
    height="10"
    className="planet-image-upload-dropzone-rect-secondary"
    viewBox="0 0 10 10"
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
  >
...
const ImageUploadDragArea: React.FC<ImageUploadDragAreaProps> = ({
  ...
  return (
    <div
      className={`planet-image-upload-drag-area ${isDragActive ? "drag-active" : ""} ${isDragOver ? "drag-over" : ""}`}
...
const ImageUploadPreview: React.FC<ImageUploadPreviewProps> = ({
  ...
  return (
    <div className="planet-image-upload-preview">
      {fileItem.status === "uploading" && (
        <div
          className="planet-image-upload-progress"
          style={{ width: `${fileItem.progress}%` }}
        />
      )}

      <div className="planet-image-upload-preview-content">
        <div className="planet-image-upload-file-info">
          <div className="planet-image-upload-file-icon">
            <CloudUploadIcon />
          </div>
          <div className="planet-image-upload-details">
            <span className="planet-image-upload-text">
              {fileItem.file.name}
            </span>
            <span className="planet-image-upload-subtext">
              {formatFileSize(fileItem.file.size)}
            </span>
          </div>
        </div>
        <div className="planet-image-upload-actions">
          {fileItem.status === "uploading" && (
            <span className="planet-image-upload-progress-text">
              {fileItem.progress}%
            </span>
          )}
          <Button
...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  <>
    <div className="planet-image-upload-dropzone">
      <FileIcon />
      <FileCornerIcon />
      <div className="planet-image-upload-icon-container">
        <CloudUploadIcon />
      </div>
    </div>

    <div className="planet-image-upload-content">
      <span className="planet-image-upload-text">
...
export const ImageUploadNode: React.FC<NodeViewProps> = (props) => {
  ...
  return (
    ...
    >
      ...
      {hasFiles && (
        <div className="planet-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="planet-image-upload-header">
